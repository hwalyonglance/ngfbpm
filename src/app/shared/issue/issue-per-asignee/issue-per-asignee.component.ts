...
@Component({
	selector: 'app-issue-per-asignee',
	templateUrl: './issue-per-asignee.component.html',
	styles: [`
	.container {
		width: 20%;
		max-width: 100%;
...
export class IssuePerAsigneeComponent implements OnInit {
	@ViewChildren(CdkDropList) cdl: QueryList<CdkDropList>
	@Input()
	get issues() { return this._issues }
	set issues(v: any) {
		console.log('set issues-per-asignee', v)
		if (v.subscribe) {
			let c = v
			c.subscribe(a=>{
				console.clear()
				console.log('v.subscribe',a)
				c=null
			})
		}
		this._issues=v
